...
const SearchedProfile = (props) => {
  ...
  useEffect(() => {
    console.log("")
    fetchData();
  }, [props.searchTerm]); // Reload the component when connection status changes
  ...
  const fetchData = async () => {
    ...
    const connectionDataPromises = data.connections.map(async (connection) => {
      const otherUserId =
        connection.userOne === user._id
          ? connection.userTwo
          : connection.userOne;
      const userResponse = await fetch(
        `http://localhost:3000/user/${otherUserId}`
      );
  ...
  const printStrangerChats = function() {
    ...
    for (let i = 0; i < strangersSearchedResult.length; i++) {
      const truncatedDisplayName =
        truncateText(strangersSearchedResult[i].displayName, 18) || "";
        ...
        <div key={i}>
          <div className="render-chat">
            <div
              className="chat"
              id={`${i}_chatStranger`}
              onClick={() => handleShowProfile(i, "stranger")}
            >
  ...
  const printConnectionChats = function() {
    ...
    for (let i = 0; i < connectionsSearchedResult.length; i++) {
      const truncatedDisplayName =
        truncateText(
          connectionsSearchedResult[i].userData.data.user.displayName,
          18
        ) || "";
      const pictureUrl = "https://example.com/random-image.png"; // Use a random image URL
        ...
        <div key={i}>
          <div className="render-chat">
            <div
              ...
              onClick={() => handleShowProfile(i, "connection")}
            >
              <img src={pictureUrl} alt="Profile" />
              <div className="chat-text" onClick={removeHiddenChatMenu}>
                <p className="contact-name">{truncatedDisplayName}</p>
                <p>
                  {connectionsSearchedResult[i]?.userData.data.user.username}
                </p>
              </div>
            </div>
