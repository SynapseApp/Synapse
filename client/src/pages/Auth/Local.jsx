...
export default function Local({ setAuthenticated, setActivePage, method }) {
  ...
  function fade_to_Auth() {
    ...
    setTimeout(() => {
      ...
      document.getElementById('Local-Backward')?.classList.remove('fade-out-r-l');
    }, 475 /*fade-out-r-l*/);
  }
  ...
  return (
    <>
      ...
      <form id="Local" onSubmit={handleSubmit}>
        ...
        {method !== 'Login' && <InputGroup type="username" placeholder="Username" name="username" setValueInput={setValueUsername} />}

        {/* Email or username input */}
        <InputGroup type={method === 'Login' ? 'email&username' : 'email'} placeholder={method === 'Login' ? 'Email or username' : 'Email'} name={method === 'Login' ? 'username' : 'email'} setValueInput={setValueEmail} />
